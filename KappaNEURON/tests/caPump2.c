...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcai = - 2.0 * ( ica - ipump ) / diam / F * ( 1e4 ) - k1 * cai * P ;
   DP = - k1 * cai * P + 2.0 * ipump / diam / F * ( 1e4 ) ;
   }
 return _reset;
}
...
static int integrate (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset=0; int error = 0;
 ...
 error = nrn_newton_thread(_newtonspace1, 2,_slist2, _p, integrate, _dlist2, _ppvar, _thread, _nt);
